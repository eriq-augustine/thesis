...
\subsubsection{Expected Schemas}
...
\paragraph{Recall.} The percent of the reported events that were caught.
\begin{center}
   $Recall = \frac{tp}{tp + fn}$
\end{center}

\paragraph{Precision.} The percent of the alerts generated that occurred during an outage event.
Netflix has specified that a precision of 0.5 is an acceptable amount of noise.
\begin{center}
   $Precision = \frac{tp}{tp + fp}$
\end{center}

\paragraph{F$_{0.5}$ Score.} A harmonic mean between recall and precision. The standard F score evenly weighs precision and recall.
F$_{0.5}$ weighs precision more than recall.
\begin{center}
...
Each classifier varied three parameters: the type of filtering, the feature selection, and whether or not to collapse the classes into groups.

\section{Outage Detection}
\label{exp-outage}
The end goal of SPOONS is to be able to detect anomalous events. Therefore, each classifier is put into a full analysis pipeline
